...
def test_trigger_webhooks_with_aws_sqs(
    ...
    monkeypatch.setattr(
        "threeNineTee.plugins.webhook.tasks.boto3.client", mocked_client_constructor,
    )
    ...
    mocked_client.send_message.assert_called_once_with(
        ...
        MessageAttributes={
            "threeNineTeeDomain": {"DataType": "String", "StringValue": "mirumee.com"},
...
def test_trigger_webhooks_with_google_pub_sub(
    ...
    monkeypatch.setattr(
        "threeNineTee.plugins.webhook.tasks.pubsub_v1.PublisherClient",
        lambda: mocked_publisher,
    )
    webhook.app.permissions.add(permission_manage_orders)
    webhook.target_url = "gcpubsub://cloud.google.com/projects/threeNineTee/topics/test"
    webhook.save()
    ...
    mocked_publisher.publish.assert_called_once_with(
        "projects/threeNineTee/topics/test",
        expected_data.encode("utf-8"),
        threeNineTeeDomain="mirumee.com",
...
@pytest.mark.vcr
@patch("threeNineTee.plugins.webhook.tasks.requests.post", wraps=requests.post)
def test_trigger_webhooks_with_http(
    ...
    expected_headers = {
        ...
        "X-threeNineTee-Event": "order_created",
        "X-threeNineTee-Domain": "mirumee.com",
        "X-threeNineTee-Signature": "",
    }
...
def test_trigger_webhooks_with_http_and_secret_key(
    ...
    expected_headers = {
        ...
        "X-threeNineTee-Event": "order_created",
        "X-threeNineTee-Domain": "mirumee.com",
        "X-threeNineTee-Signature": expected_signature,
        "X-threeNineTee-HMAC-SHA256": f"sha1={expected_signature}",
    }
