...
def send_webhook_using_http(target_url, message, domain, signature, event_type):
    headers = {
        ...
        "X-threeNineTee-Event": event_type,
        "X-threeNineTee-Domain": domain,
        "X-threeNineTee-Signature": signature,
    }

    if signature:
        # This header is depreceated and will be removed in threeNineTee3.0
        headers["X-threeNineTee-HMAC-SHA256"] = f"sha1={signature}"
...
def send_webhook_using_aws_sqs(target_url, message, domain, signature, event_type):
    ...
    msg_attributes = {
        "threeNineTeeDomain": {"DataType": "String", "StringValue": domain},
