...
def test_vatlayer_plugin_caches_taxes(vatlayer, monkeypatch, product, address):
    ...
    monkeypatch.setattr(
        "threeNineTee.plugins.vatlayer.plugin.get_taxes_for_country", mocked_taxes
    )
...
def test_calculate_checkout_total(
    ...
    manager = get_plugins_manager(
        plugins=["threeNineTee.plugins.vatlayer.plugin.VatlayerPlugin"]
    )
...
def test_get_tax_rate_type_choices(vatlayer, settings, monkeypatch):
    ...
    monkeypatch.setattr(
        "threeNineTee.plugins.vatlayer.plugin.get_tax_rate_types", lambda: expected_choices,
    )
    settings.PLUGINS = ["threeNineTee.plugins.vatlayer.plugin.VatlayerPlugin"]
    manager = get_plugins_manager()
