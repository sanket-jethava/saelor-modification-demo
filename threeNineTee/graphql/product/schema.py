...
from .types.products import AllCategoryList,AllCategoryNameList
from ...product import models
from ..core.utils import from_global_id_strict_type

class ProductQueries(graphene.ObjectType):
    all_categories = graphene.List(AllCategoryList)

    def resolve_all_categories(self, info):
        if AdminAuthorization(info):
            return resolve_all_categories(info)

    all_categories_name = graphene.List(AllCategoryNameList)

    def resolve_all_categories_name(self, info):
        if AdminAuthorization(info):
            return resolve_all_categories_name(info)
    ...
    attribute_values = FilterInputConnectionField(
        AttributeValue,
        description="List of the shop's attributes values",
    )

    attribute_value = graphene.Field(
        AttributeValue,
        id=graphene.Argument(
            graphene.ID, description="ID of the attribute value", required=True
        ),
        description="Look up an attribute value by ID.",
    )
    ...
    def resolve_attribute_values(self, info, **kwargs):
        if AdminAuthorization(info):
            return resolve_attribute_values(info, **kwargs)

    def resolve_attribute_value(self, info, id):
        if AdminAuthorization(info):
            return graphene.Node.get_node_from_global_id(info, id, AttributeValue)
    ...
    def resolve_product(self, info, id=None, slug=None):
        if AdminAuthorization(info):
            validate_one_of_args_is_in_query("id", id, "slug", slug)
            if id:
                product_id = from_global_id_strict_type(id, only_type="Product", field="pk")
