...
class AttributeValueCreateInput(graphene.InputObjectType):
    ...
    code=graphene.String(required=False,description="color code")
    value=graphene.String(required=True,description="Default Store View")
...
class AttributeCreateInput(graphene.InputObjectType):
    ...
    attribute_code=graphene.String(required=False,description=AttributeDescriptions.CODE)
    ...
    def perform_mutation(cls, _root, info, **data):
        try:
            if AdminAuthorization(info):
                ...
                return AttributeCreate(message="Attribute added successfully!",attribute=instance)
            else:
                return AttributeCreate(message="Only admin have rights to perform this task.")
        except Exception as e:
            return AttributeCreate(message=e)
    ...
    def perform_mutation(cls, _root, info, id, input):
        try:
            if AdminAuthorization(info):
                ...
                # Return the attribute that was created
                return AttributeUpdate(message="Attribute updated successfully!",attribute=instance)
            else:
                return AttributeUpdate(message="Only admin have rights to perform this task.")
        except Exception as e:
            return AttributeUpdate(message=e)
    ...
    def perform_mutation(cls, _root, info, attribute_id, input):
        try:
            if AdminAuthorization(info):
                ...
                instance.save()
                cls._save_m2m(info, instance, cleaned_input)
                return AttributeValueCreate(message="Attribute value added successfully!", attributeValue=instance)
            else:
                return AttributeValueCreate(message="Only admin have rights to perform this task.",attributeValue=instance)
        except Exception as e:
            print(e)
            return AttributeValueCreate(message=e)
