...
from .resolvers import resolve_checkout, resolve_checkout_lines, resolve_checkouts, resolve_checkout_addresses, resolve_checkout_address
from .types import Checkout, CheckoutLine, CheckoutAddress


class CheckoutQueries(graphene.ObjectType):
    # Checkout addresses
    checkout_addresses = graphene.List(CheckoutAddress, 
        uid = graphene.Argument(graphene.ID, description="user id for filter and fetch data of user.", required=False)
        )

    def resolve_checkout_addresses(self, info, uid):
            return resolve_checkout_addresses(info, uid)

    #testing
    checkout_address = graphene.Field(CheckoutAddress, 
        uid = graphene.Argument(graphene.ID, description="user id for filter and fetch data of user.", required=False)
        )

    def resolve_checkout_address(self, info, uid):
            return resolve_checkout_address(info, uid) 
...
class CheckoutMutations(graphene.ObjectType):
    ...
    add_new_address = addNewAddress.Field()
