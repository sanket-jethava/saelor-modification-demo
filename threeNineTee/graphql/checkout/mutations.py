...
import requests
from ..account.enums import CountryCodeEnum, CustomerEventsEnum

from ..all_user_authorization import AdminAuthorization, ArtistAuthorization, UserAuthorization
...
#Add new address (B2C)
class addNewAddressInput(graphene.InputObjectType):
    user_id = graphene.ID(required=True)
    first_name = graphene.String(required=True)
    country = CountryCodeEnum(required=True)
    ...
    type_of_address = graphene.String(required=True)
    # is_billing = graphene.Boolean(required=True)

    billing_is_same = graphene.Boolean(required=True)

    first_name_s = graphene.String(required=False)
    country_s = CountryCodeEnum(required=False)
    country_code_s = graphene.String(required=False)
    phone_s = graphene.String(required=False)
    address_s = graphene.String(required=False)
    state_s = graphene.String(required=False)
    city_s = graphene.String(required=False)
    zip_code_s = graphene.String(required=False)
    type_of_address_s = graphene.String(required=False)

    
class addNewAddress(graphene.Mutation):
    class Arguments:
        input = addNewAddressInput(required=False)
    ...
    def mutate(cls, root, info, **data):
        try:
            if UserAuthorization(info):
                user_id = data.get('input')['user_id']
                first_name = data.get('input')['first_name']
                country = data.get('input')['country']
                country_code = data.get('input')['country_code']
                number = data.get('input')['phone']
                address = data.get('input')['address']
                state = data.get('input')['state']
                city = data.get('input')['city']
                zip_code = data.get('input')['zip_code']
                type_of_address = data.get('input')['type_of_address']

                billing_is_same = data.get('input')['billing_is_same']

                phone = "+" + country_code + number

                mutation = """
                mutation address($user_id: ID!, $first_name: String, $streetAddress1: String, $city: String, $zipcode: String, $country: CountryCode, $phone: String, $state: String, $type_of_address: String){
                addressCreate(input:{
                ...
                variables = {'user_id':user_id,'first_name':first_name, 'streetAddress1': address, 'city':city, 'zipcode':zip_code, 'country':country, 'phone':phone, 'state':state, 'type_of_address': type_of_address}
                # variables = {'first_name':first_name, 'streetAddress1': address, 'city': city, 'postalCode': zip_code, 'country': country,  'phone': phone}
                ...
                # request = requests.post("http://localhost:8000/graphql/", json={'query': mutation})
                result = request.json()
                # print(result)
                try:
                    address_id = result["data"]["addressCreate"]["address"]["id"]
                    address_int_id = from_global_id_strict_type(address_id, only_type="Address", field="pk")
                except:
                    return addNewAddress(message="This address is already registered!")
                # print(address_int_id)

                if billing_is_same is False:
                    #for billing address
                    first_name_s = data.get('input')['first_name_s']
                    country_s = data.get('input')['country_s']
                    country_code_s = data.get('input')['country_code_s']
                    phone_s = data.get('input')['phone_s']
                    address_s = data.get('input')['address_s']
                    state_s = data.get('input')['state_s']
                    city_s = data.get('input')['city_s']
                    zip_code_s = data.get('input')['zip_code_s']
                    type_of_address_s = data.get('input')['type_of_address_s']

                    phone_s = "+" + country_code + phone_s
                    
                    mutation = """
                    mutation address($user_id: ID!, $first_name: String, $streetAddress1: String, $city: String, $zipcode: String, $country: CountryCode, $phone: String, $is_billing: Boolean, $shipping_id: ID!, $state: String, $type_of_address: String){
                        addressCreate(input:{
                        firstName:$first_name
                        lastName:""
                        companyName:""
                    ...
                    """
                    # print("1.",address_id)
                    # print("2.",address_int_id)

                    variables = {'user_id':user_id,'first_name':first_name_s, 'streetAddress1': address_s, 'city':city_s, 'zipcode':zip_code_s, 'country':country_s, 'phone':phone_s, 'is_billing': True, 'shipping_id': address_id, 'state':state_s, 'type_of_address': type_of_address_s}
                    # variables = {'first_name':first_name, 'streetAddress1': address, 'city': city, 'postalCode': zip_code, 'country': country,  'phone': phone}
                    # client.execute(mutation, variables)
                    request = requests.post("http://localhost:8000/graphql/", json={'query': mutation, 'variables': variables})
                    # request = requests.post("http://localhost:8000/graphql/", json={'query': mutation})
                    ...
                    address_instance = models.Address.objects.get(id = address_int_id)
                    address_instance.shipping_id = address_int_id
                    # address_instance.is_billing = False
                ...
                return addNewAddress(message=message)

        except Exception as e:
            return addNewAddress(message=e)
