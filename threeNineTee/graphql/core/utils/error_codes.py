...
threeNineTee_error_codes = []
for enum in threeNineTee_ERROR_CODE_ENUMS:
    threeNineTee_error_codes.extend([code.value for code in enum])
...
def get_error_code_from_error(error) -> str:
    ...
    if isinstance(code, Enum):
        code = code.value
    if code not in threeNineTee_error_codes:
        return "invalid"
    return code
