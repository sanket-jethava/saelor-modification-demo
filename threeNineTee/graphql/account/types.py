...
class ArtistSocialMediaDetail(DjangoObjectType):
    class Meta:
        model = models.SocialMediaLinks
        fields = ('dribble_link','facebook_link','twitter_link','instagram_link','behance_link')


class MobileOTP(DjangoObjectType):
    class Meta:
        model = models.loginOTP
        fields = ('otp','mobile')

class PackageDetail(DjangoObjectType):
    class Meta:
        model = models.Package
        fields = ('title','description','document_link','date_created','is_active','price')

class LoginDetail(DjangoObjectType):
    class Meta:
        model = models.LoginSummary
        fields = ('date','login_time','logout_time','device_type')
...
class AddressInput(graphene.InputObjectType):
    ...
    is_billing = graphene.Boolean(required=False, description="Is this billing")
    shipping_id = graphene.ID(required=False, description="Shipping id")
    type_of_address = graphene.String(description="Type of address.")
...
class CustomerEvent(CountableDjangoObjectType):
    ...
    order = graphene.Field(
        "threeNineTee.graphql.order.types.Order", description="The concerned order."
    )
    order_line = graphene.Field(
        "threeNineTee.graphql.order.types.OrderLine", description="The concerned order line."
    )
...
class UserPermission(Permission):
    source_permission_groups = graphene.List(
        graphene.NonNull("threeNineTee.graphql.account.types.Group"),
...
class User(CountableDjangoObjectType):
    ...
    gift_cards = PrefetchingConnectionField(
        "threeNineTee.graphql.giftcard.types.GiftCard",
    ...
    orders = PrefetchingConnectionField(
        "threeNineTee.graphql.order.types.Order", description="List of user's orders."
    )
    ...
    permissions = graphene.List(
        ...
        deprecation_reason=(
            "Will be removed in threeNineTee 2.11." "Use the `userPermissions` instead."
        ),
    )
    ...
    permission_groups = graphene.List(
        "threeNineTee.graphql.account.types.Group",
    ...
    stored_payment_sources = graphene.List(
        "threeNineTee.graphql.payment.types.PaymentSource",
...
class StaffNotificationRecipient(CountableDjangoObjectType):
    ...
    class Meta:
        description = (
            "Represents a recipient of email notifications send by threeNineTee, "
...
#B2B sub users
class SubUserDetail(CountableDjangoObjectType):
    social = graphene.List(ArtistSocialMediaDetail, description="social media links")
    class Meta:
        model = models.User
        fields = ('id', 'first_name', 'last_name', 'country_code', 'phonenumber', 'date_of_birth', 'email', 'company_name', 'contact_person', 'pan_number', 'gst_number')
        interfaces = (relay.Node, )

    def resolve_social(info,uid):
        return models.SocialMediaLinks.objects.filter(user_id=info.id)

class UserDetail(DjangoObjectType):
    class Meta:
        model = models.User
        fields = ('id','upi_id','bio','social_media_id','jwt_token_key','note','last_login','date_joined','private_metadata','metadata','avatar','is_staff','is_superuser','is_active','date_of_birth','last_name','first_name','is_artist','available_token','pan_number','email','gst_number','gender','package_id','phonenumber','is_loginwith_google','is_loginwith_facebook')
        interfaces = (relay.Node, )