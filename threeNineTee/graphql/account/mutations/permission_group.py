...
class StaffUserCreateInput(graphene.InputObjectType):
    ...
    def mutate(cls, root, info, **data):
        try:
            if AdminAuthorization(info):
                message =""
                add_permissions = data.get('input')['add_permissions']
                ...
                from ....account import models
                try:
                    user = models.User.objects.create(is_staff=True,email=email,first_name=first_name,last_name=last_name,phonenumber=phone_number)
                    user.set_password(password)
                    
                    clean_data = [i.split(".")[1] for i in add_permissions]
                    ...
                    user.save()
                    message = "Staff user created with email - "+user.email+"."
                    return StaffUserCreate(message=message)
                except:
                    return StaffUserCreate(message="User exists with same email or phone number.")
        except Exception as e:
            return StaffUserCreate(message=e)

class StaffUserUpdatePermissionInput(graphene.InputObjectType):
    add_permissions = graphene.List(
        graphene.NonNull(PermissionEnum),
        description="List of permission code names to assign to this group.",
        required=False,
...
class StaffUserUpdatePermission(graphene.Mutation):
    class Arguments:
        input = StaffUserUpdatePermissionInput(
            description="Input fields to create permission group.", required=True
        )
    ...
    def mutate(cls, root, info, **data):
        try:
            if AdminAuthorization(info):
                ...
                message = "Permission added to user with this registered email - "+user.email+"."
                return StaffUserUpdatePermission(message=message)
                
        except Exception as e:
            return StaffUserUpdatePermission(message=e)
