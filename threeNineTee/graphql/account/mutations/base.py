...
from ...all_user_authorization import CommonAuthorization
...
class PasswordChange(BaseMutation):
    ...
    class Arguments:
        email = graphene.String(required = True,description="Current user email.")
...
class BaseAddressUpdate(graphene.Mutation):
    ...
    address = graphene.Field(Address,description= "Updated address data.")

    @classmethod
    def mutate(cls, root, info, **data):
        cleaned_input = data.get("input")
        address_id = data.get("id")
        address_id = from_global_id_strict_type(address_id, only_type="Address", field="pk") 
        address_obj = models.Address.objects.get(id=address_id)
        for (key, value) in cleaned_input.items():
            setattr(address_obj, key, value)
        address_obj.save()
        return BaseAddressUpdate(address_obj)
