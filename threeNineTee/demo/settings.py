"""Settings file to run threeNineTee in "demo" mode.
...
variables are set (see the `threeNineTee.core.utils.random_data.configure_braintree` function
for more details)
...
ROOT_URLCONF = "threeNineTee.demo.urls"

PLUGINS += ["threeNineTee.plugins.anonymize.plugin.AnonymizePlugin"]

GRAPHENE["MIDDLEWARE"].insert(0, "threeNineTee.graphql.middleware.ReadOnlyMiddleware")  # type: ignore
...
PWA_ORIGINS = get_list(os.environ.get("PWA_ORIGINS", "pwa.threeNineTee.io"))
PWA_DASHBOARD_URL_RE = re.compile("^https?://[^/]+/dashboard/.*")

ROOT_EMAIL = os.environ.get("ROOT_EMAIL")

# Remove "threeNineTee.core" and add it after adding "threeNineTee.demo", to have "populatedb"
# command overriden when using demo settings
# (see threeNineTee.demo.management.commands.populatedb).
INSTALLED_APPS.remove("threeNineTee.core")
INSTALLED_APPS += ["threeNineTee.demo", "threeNineTee.core"]
...
def before_send(event: dict, _hint: dict):
    ...
    if ev_logger_name != "threeNineTee.graphql.errors.handled":
        return event
